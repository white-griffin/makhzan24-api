    ...
    <div class="card" data-select2-id="select2-data-131-rhmf">
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1"> فهرست جملات  کاربران</span>
            </h3>
                            ...
                            @foreach($sentences as $sentence)
                                <tr class="odd" id="{{$sentence->id}}" data-id="{{$sentence->id}}" style="border-color: black">
                                    ...
                                    <td class="">
                                        ...
                                        @if($sentence->status != \App\Constants\Constant::RESTRICTED)
                                            <a class="btn btn-icon btn-light-danger btn-sm me-1" title="ثبت گزارش" id="restrict_btn" onclick="restrictSentence({{$sentence->id}})">
                                             <span class="svg-icon svg-icon-3">
                                                <i class="fa fa-ban"></i>
                                            </span>
                                            </a>
...
@section('scripts')
    ...
    <script>
        function restrictSentence(sentenceId) {
            Swal.fire({
                title: "از ممنوع کردن جمله مطمئن هستید ؟",
                text: "متن جمله : " + document.getElementById('fa_'+sentenceId).value,
                icon: "warning",
                showCancelButton: true,
                cancelButtonText: 'لغو',
                confirmButtonText: 'بله',
                confirmButtonColor: "rgb(21 153 27)",
                    ...
                    Swal.fire({
                        ...
                        didOpen: () => {
                            ...
                            try {
                                ...
                                $.ajax({
                                    url: "{{route('admin.sentence.restrict-sentence')}}",
                                    method: 'POST',
                                    data: {
                                        _token: "{!! csrf_token() !!}",
